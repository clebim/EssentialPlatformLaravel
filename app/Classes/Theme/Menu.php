...
class Menu
{
    ...
    public static function getPagesAside() {
        $accessId = AccessGroup::where('privilege_id', Auth::user()->privilege_id)->pluck('page_id');
        $pages = Page::whereIn('id', $accessId)->get();

        $finalArray = [];
        $sections = [];

        foreach($pages as $page){
            if(!in_array($page->section->section, $sections)) {
                array_push($sections, $page->section->section);
            }
        }
        
        for($i = 0; $i < count($sections); $i++) {

            array_push($finalArray, ['section' => $sections[$i]]);
            foreach($pages as $page){
                $auxArray = [];
                $subMenu = []; 
                if($page->section->section == $sections[$i] && $page->page_id == 0 && $page->fl_active == 1) {
                    $auxArray = [
                        'title' => $page->nm_page,
                        'root' => true,
                        'icon' => $page->ic_page,
                    ...
                    for($j = 0; $j < count($pages); $j++) {
                        if($pages[$j]->page_id == $page->id && $pages[$j]->page_id != 0 && $pages[$j]->fl_active == 1) {
                            $aux = [
                                'title' => $pages[$j]->nm_page,
                                'page' => $pages[$j]->ds_url
                            ];
                            array_push($subMenu, $aux);
                        }
                    }

                    if(count($subMenu) != 0) {
                        $auxArray['submenu'] = $subMenu;
                    }
                    array_push($finalArray, $auxArray);
                }
            }
        }
        return [
